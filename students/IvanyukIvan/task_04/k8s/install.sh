...
echo "=== ЛР04: Установка мониторинга и приложения (вариант 32) ==="
...
echo "Устанавливаем приложение (вариант 32)..."
helm upgrade --install app32 ./helm/monitoring-app \
...
echo ""
echo "Приложение:"
if kubectl get ingress -n "$APP_NAMESPACE" app32-monitoring-app >/dev/null 2>&1; then
  INGRESS_HOST=$(kubectl get ingress -n "$APP_NAMESPACE" app32-monitoring-app -o jsonpath='{.spec.rules[0].host}')
  ...
  echo "   kubectl port-forward svc/app32-monitoring-app 5000:5000 -n $APP_NAMESPACE"
fi
