...
```
┌─────────────────────────────────────────────────────────────┐
│ Namespace: state-as63-220024-v19                            │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  ┌──────────────────────────────────────────────────────┐   │
│  │ StatefulSet: db-postgres-as63-220024-v19             │   │
│  │ ┌────────────────────────────────────────────────┐   │   │
│  │ │ Pod: db-postgres-as63-220024-v19-0             │   │   │
│  │ │ ┌────────────────────────────────────────────┐ │   │   │
│  │ │ │ Container: postgres:15-alpine              │ │   │   │
│  │ │ │ Port: 5432                                 │ │   │   │
│  │ │ │ ENV: STU_ID=220024, STU_GROUP=АС-63       │ │   │   │
│  │ │ └────────────────────────────────────────────┘ │   │   │
│  │ └────────────────────────────────────────────────┘   │   │
│  └──────────────────────────────────────────────────────┘   │
│                        │                                      │
...
│  │ Port: 5432                                           │   │
│  │ DNS: db-postgres-as63-220024-v19-0.db-postgres-     │   │
│  │      headless.state-as63-220024-v19.svc.cluster.local│   │
│  └──────────────────────────────────────────────────────┘   │
│                                                               │
│  ┌──────────────────────────────────────────────────────┐   │
│  │ PersistentVolumeClaim: postgres-data-*               │   │
│  │ Size: 3Gi, StorageClass: standard-storage            │   │
...
│                                                               │
│  ┌──────────────────────────────────────────────────────┐   │
│  │ CronJob: backup-postgres-as63-220024-v19             │   │
...
│                                                               │
│  ┌──────────────────────────────────────────────────────┐   │
│  │ Job: restore-postgres-as63-220024-v19                │   │
...
|----------|----------|-------------|
| **Database** | PostgreSQL 15-alpine | Вариант 19, легковесный образ |
| **Replicas** | 1 | Достаточно для демонстрации |
| **Data PVC** | 3Gi, standard-storage, RWO | Согласно варианту, Retain policy |
| **Backup PVC** | 1Gi, standard, RWX | Для хранения backup файлов |
| **Backup schedule** | "0 18 \* \* \* " | Согласно варианту 19 (каждый день в 18:00) |
...
kubectl wait --for=condition=ready pod -l app=postgres \
  -n state-as63-220024-v19 --timeout=120s

# Проверить статус
kubectl get all,pvc -n state-as63-220024-v19
```
...
Все ресурсы успешно развернуты в namespace `state-as63-220024-v19`:
- StatefulSet: `db-postgres-as63-220024-v19` (READY 1/1)
- Pod: `db-postgres-as63-220024-v19-0` (Running, 1/1)
- Headless Service: `db-postgres-headless` (ClusterIP: None)
- CronJob: `backup-postgres-as63-220024-v19` (расписание: `0 18 * * *`)
- Завершенные backup Jobs (3 шт., Complete)
- PVC: `postgres-data-*` (3Gi, standard-storage, Bound)
- PVC: `backup-postgres-pvc` (1Gi, standard, Bound)
- StorageClass: `standard-storage` с политикой Retain
...
- `id=1`, `student_id=220024`
- `fullname=Соколова Маргарита Александровна`
- `university_group=АС-63`, `variant=19`
...
1. Pod `db-postgres-as63-220024-v19-0` удален командой `kubectl delete pod`
2. StatefulSet автоматически пересоздал pod (~30 секунд)
3. Новый pod успешно запустился (Running 1/1)
4. **Данные полностью сохранились** - таблица `students` содержит запись с `student_id=220024`
...
- Логи backup показывают:
  - Student ID: 220024, Group: АС-63, Variant: 19
  - Timestamp: 2025-12-04 18:00:00
  - ✅ Backup completed: `/backups/backup_20251204_180000.sql.gz` (4.0K)
- Файл успешно сохранен в PVC `backup-postgres-pvc`

✅ **Критерий выполнен:** CronJob с расписанием `0 18 * * *` создан и работает корректно.
...
5. **Данные полностью восстановлены** из backup - запись с `id=1`, `student_id=220024`, `fullname=Соколова Маргарита Александровна` снова присутствует в таблице
...
kubectl get all,pvc,secret,storageclass standard-storage -n state-as63-220024-v19
```
...
pod/db-postgres-as63-220024-v19-0            1/1     Running   0          5m

NAME                           TYPE        CLUSTER-IP   EXTERNAL-IP   PORT(S)
service/db-postgres-headless   ClusterIP   None         <none>        5432/TCP

NAME                                            READY   AGE
statefulset.apps/db-postgres-as63-220024-v19   1/1     5m

NAME                                                         SCHEDULE     SUSPEND   ACTIVE
cronjob.batch/backup-postgres-as63-220024-v19   0 18 * * *   False     0

NAME                                                                STATUS   VOLUME                                     CAPACITY
persistentvolumeclaim/backup-postgres-pvc                           Bound    pvc-xxx   1Gi
persistentvolumeclaim/postgres-data-db-postgres-as63-220024-v19-0   Bound    pvc-yyy   3Gi
```
...
```bash
# Получить имя пода
POD=$(kubectl get pod -n state-as63-220024-v19 -l app=postgres -o jsonpath='{.items[0].metadata.name}')

# Создать таблицу и добавить данные
kubectl exec -n state-as63-220024-v19 $POD -- psql -U admin -d testdb -c "
...
VALUES ('220024', 'Соколова Маргарита Александровна', 'АС-63', 19);
"
...
----+------------+------------------------------------+------------------+---------+----------------------------
  1 | 220024     | Соколова Маргарита Александровна  | АС-63            |      19 | 2025-12-04 16:00:00.000000
(1 row)
```
...
```bash
# 1. Удалить pod
kubectl delete pod db-postgres-as63-220024-v19-0 -n state-as63-220024-v19
...
# 3. Проверить данные
kubectl exec -n state-as63-220024-v19 \
  $(kubectl get pod -n state-as63-220024-v19 -l app=postgres -o jsonpath='{.items[0].metadata.name}') \
...
✅ Данные сохранились (запись с student_id=220024 присутствует)
...
```
pod "db-postgres-as63-220024-v19-0" deleted
pod/db-postgres-as63-220024-v19-0 condition met

 id | student_id |              fullname              | university_group | variant
----+------------+------------------------------------+------------------+---------
  1 | 220024     | Соколова Маргарита Александровна  | АС-63            |      19
(1 row)
```
...
```bash
kubectl get cronjob -n state-as63-220024-v19
kubectl get jobs -n state-as63-220024-v19
```
...
# Или напрямую
kubectl create job --from=cronjob/backup-postgres-as63-220024-v19 \
  backup-manual-$(date +%Y%m%d-%H%M%S) -n state-as63-220024-v19

# Проверить статус
kubectl get jobs -n state-as63-220024-v19

# Посмотреть логи
kubectl logs -n state-as63-220024-v19 -l app=postgres-backup --tail=50
```
...
Starting backup...
Backup file: /backups/backup_20251204_180000.sql.gz
Backup size: 1.2KB
...
# Удалить таблицу
kubectl exec -n state-as63-220024-v19 $POD -- \
...
kubectl wait --for=condition=complete job/restore-postgres-as63-220024-v19 \
  -n state-as63-220024-v19 --timeout=120s

# Посмотреть логи
kubectl logs -n state-as63-220024-v19 -l app=postgres-restore
```
...
Group: АС-63
Variant: 19
Restoring from: /backups/backup_20251204_180000.sql.gz
...
|--------|-----|--------|--------|
| **Namespace** | state-as63-220024-v19 | Active | Slug: as-63-220024-v19 |
| **StatefulSet** | db-postgres-as63-220024-v19 | Running 1/1 | PostgreSQL 15-alpine |
| **Service** | db-postgres-headless | Active | clusterIP: None |
| **PVC data** | postgres-data-* | Bound | 3Gi, standard-storage, RWO |
| **PVC backup** | backup-postgres-pvc | Bound | 1Gi, standard, RWX |
| **Secret** | db-postgres-secret | Created | POSTGRES_USER/PASSWORD/DB |
| **StorageClass** | standard-storage | Active | Retain policy, dynamic provisioning |
| **CronJob** | backup-postgres-as63-220024-v19 | Created | Schedule: "0 18 \* \* \*" |
...
1. ✅ **StatefulSet для PostgreSQL** с volumeClaimTemplates (3Gi, standard-storage)
2. ✅ **Headless Service** (clusterIP: None) для стабильных DNS-имён
3. ✅ **Динамическое хранилище** через StorageClass с политикой Retain
4. ✅ **Автоматическое резервное копирование** через CronJob (расписание "0 18 \* \* \*")
