...
В этой работе студенты развёртывают stateful-приложение в Kubernetes: настройка StatefulSet, использование PersistentVolumeClaim/PersistentVolume для постоянного хранения данных, создание Headless Service для стабильных DNS-имён под'ов, а также реализация резервного копирования и восстановления данных с помощью Job/CronJob. Документы и примеры находятся в каталоге `tasks/task_03/`.
...
* Создать Headless Service для прямого доступа к под'ам через DNS.
* Реализовать механизм резервного копирования (backup) и восстановления (restore) данных.
* Проверить сохранность данных после перезапуска под'ов.
...
1. Выбрать stateful-сервис согласно варианту (Postgres, Redis или MinIO) и подготовить манифесты:
    ...
    * Headless Service (clusterIP: None) для стабильных DNS-имён под'ов;
...
2. Развернуть stateful-приложение в Kubernetes:
    * Применить все манифесты и убедиться, что под'ы запущены и PVC создан;
...
* Проверка сохранности данных после перезапуска под'ов (демонстрация с логами/скриншотами) — 20
