...
Экспортируемые метрики (с префиксом `app42_`):
...
```
2. Установить приложение (локально из каталога `chart`):
```bash
helm install app31 ./chart -n app42 --create-namespace
kubectl get pods -n app42
```
3. Проверить, что `ServiceMonitor` и `PrometheusRule` созданы в `monitoring` namespace и что Prometheus видит метрики:
```bash
kubectl get servicemonitors -n monitoring
kubectl get prometheusrules -n monitoring
# В Prometheus: проверка expression и targets
```
4. Откройте Grafana (через minikube или port-forward), создайте/импортируйте дашборды по availability/latency и проверьте визуализацию и историю срабатываний алертов.
