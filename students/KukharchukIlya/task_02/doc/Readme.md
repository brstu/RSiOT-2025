...
- Группа - АС-63
- № студенческого/зачетной книжки (StudentID) - 220017
...
```bash
sudo sh -c "echo '127.0.0.1 web13.local' >> /etc/hosts"
```
...
```bash
http://web13.local/visit
```

Студент: 220017, Группа: АС-63, Вариант: 13, Кол-во визитов: 4
...
```bash
kubectl logs <db-pod-name> -n app13
```

#### 4.2 Просмотр логов аpp

```bash
kubectl logs <app-name> -n app13
```
