...
**Студент:** Ващук Анатолий Валерьевич  
**Группа:** АС-63  
**Вариант:** 1
...
| Параметр | Значение |
|--------|---------|
| Префикс метрик | `app01_` |
| SLO доступность | 99.0% |
| SLO p95 latency | ≤ 300 ms |
| Alert | 5xx > 2% за 10 минут |
...
```powershell
helm install monitoring-app ./helm/monitoring-app `
...
|------|------|---------|
| `app01_http_requests_total` | Counter | Количество HTTP-запросов |
| `app01_http_request_duration_seconds` | Histogram | Время обработки запросов |
...
- **Условие:** доступность < 99.0%
- **Период:** 10 минут
- **Severity:** `critical`

```promql
(sum(rate(app01_http_requests_total{status!~"5.."}[10m])) 
/ sum(rate(app01_http_requests_total[10m]))) * 100 < 99
```
...
histogram_quantile(
  0.95,
  sum(rate(app01_http_request_duration_seconds_bucket[10m])) by (le)
) > 0.3
```
...
```promql
rate(app01_http_requests_total[5m])
```

```promql
(sum(rate(app01_http_requests_total{status!~"5.."}[10m])) 
/ sum(rate(app01_http_requests_total[10m]))) * 100
```
...
Реализована система наблюдаемости приложения с контролем **SLO** для **варианта 1**.  
