...
- **Вариант №:** 41
- **Дата выполнения:** 11 декабря 2025 г.
- **Slug:** as64-220053-v41
...
- `APP_PORT` - порт приложения (по умолчанию 7991)
- `STU_ID` - ID студента (220053)
- `STU_GROUP` - группа (АС-64)
- `STU_VARIANT` - номер варианта (41)
- `APP_NAME` - имя приложения (web41)
- `APP_NAMESPACE` - namespace (app41)

**Логирование:**

```text
=== ЗАПУСК ПРИЛОЖЕНИЯ ===
Студент: 220053, Группа: АС-64, Вариант: 41
Приложение: web41, Namespace: app41
Порт: 7991
Время запуска: 2025-12-08T10:00:00Z
Сервер запущен на порту 7991
...
Создан namespace `app41` со всеми необходимыми labels и annotations:
...
- **Replicas:** 2 (согласно варианту 41)
- **Strategy:** RollingUpdate с maxSurge=1, maxUnavailable=1
- **Image:** KotyaLapka/web41:latest
- **Security Context:**
  - `runAsNonRoot: true`
  - `runAsUser: 1000`
...
**Resources (согласно варианту 41):**
...
- Deployment: `app-as64-220053-v41`
- Все labels содержат slug: `as64-220053-v41`
...
- **Name:** `net-as64-220053-v41` (префикс net- согласно требованиям)
- **Port:** 7991
- **Selector:** соответствует labels в Deployment

```yaml
selector:
  app.kubernetes.io/name: web41
  app.kubernetes.io/instance: as64-220053-v41
```
...
```bash
docker build -t KotyaLapka/web41:latest ./src
kind load docker-image KotyaLapka/web41:latest --name lab02-cluster
```
...
Откройте в браузере: `http://localhost:7991`
...
Events:
  ...
  ----    ------     ----  ----               -------
  Normal  Scheduled  5m    default-scheduler  Successfully assigned app41/app-as64-220053-v41-xxx to lab02-cluster-control-plane
  Normal  Pulled     5m    kubelet            Container image "KotyaLapka/web41:latest" already present on machine
  Normal  Created    5m    kubelet            Created container web41
  Normal  Started    5m    kubelet            Started container web41
```
...
```bash
kubectl set image deployment/app-as64-220053-v41 web41=KotyaLapka/web41:v2 -n app41
```

#### 9.2. Наблюдение за процессом

```bash
kubectl rollout status deployment/app-as64-220053-v41 -n app41
```
...
```bash
kubectl logs -n app41 -l app.kubernetes.io/name=web41 --tail=100
```
...
```bash
kubectl delete pod -n app41 -l app.kubernetes.io/name=web41 --force=false
```
...
```bash
kubectl get deployment app-as64-220053-v41 -n app41 --show-labels
```
...
- `org.bstu.student.fullname: "Смердина Анастасия Валентиновна"`
- `org.bstu.student.email: "AS006424@g.bstu.by"`
...
```bash
cd task_02/src
docker build -t annkrq/web41:latest .
```
...
```bash
helm upgrade --install web41 ./helm/web41 --create-namespace
```
...
Откройте браузер: <http://localhost:7991>
...
# Info endpoint
kubectl run curl-test --image=curlimages/curl:latest --rm -i --restart=Never -n app41 \
  -- curl -s http://net-aas64-220053-v41:7991/info
```

#### 8. Просмотр логов

```bash
kubectl logs -n app41 -l app.kubernetes.io/name=web41 --tail=50 --follow
```
...
make port-forward   # Пробросить порт 7991
