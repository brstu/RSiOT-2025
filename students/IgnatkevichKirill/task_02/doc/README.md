...
<p align="right">Игнаткевич Кирилл Сергеевич</p>
...
labels:
  ...
  org.bstu.variant: "09"
  org.bstu.student.fullname: "Ignatkevich Kirill Sergeevich"
...
```powershell
cd students/IgnatkevichKirill/task_02/src
docker build -t pyrokekw/lab02-web09:stu-220042-v09 .
docker run -d -p 8071:8071 --name test-web09 pyrokekw/lab02-web09:stu-220042-v09
...
- Namespace: `app09`
- Deployment: `app-web09` — `replicas: 2`, image `pyrokekw/lab02-web09:latest` (замените на собранный образ), порт `8071`.
- Service: `net-web09` — `ClusterIP`, port 80 → `targetPort: 8071`.
- Ingress: `ingress-web09` с `ingressClass: nginx`, host `web09.local`.
- ConfigMap: `config-web09` содержит `PORT=8071`, `STU_ID=220042`, `STU_GROUP=as-64`, `STU_VARIANT=09`.
...
cd students/IgnatkevichKirill/task_02/src
docker build -t pyrokekw/lab02-web09:latest .
kind load docker-image pyrokekw/lab02-web09:latest --name lab2
