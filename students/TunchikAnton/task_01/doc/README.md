...
1. **MD029/ol-prefix** - В разделе "Как запустить" нумерация теперь правильная:
   - `1.` (вместо `2.`)
   - `2.` (вместо `3.`)
   - `3.` (вместо `4.`)

2. **MD047/single-trailing-newline** - Добавлена пустая строка в конце файла

**Причина проблемы**: Когда между пунктами упорядоченного списка вставляются блоки кода (```), некоторые парсеры Markdown сбивают нумерацию. Решение - использовать правильную последовательную нумерацию 1, 2, 3 независимо от содержимого между пунктами.