...
# Переменные
$DOCKER_IMAGE = "KotyaLapka/web41:latest"
$NAMESPACE = "app41"
...
function Deploy-Application {
    ...
    helm upgrade --install web41 ./src/helm/web41 --create-namespace --wait --timeout 5m
...
function Test-Application {
    ...
    kubectl run curl-test --image=curlimages/curl:latest --rm -i --restart=Never -n $NAMESPACE -- curl -s http://net-as64-220053-v41:7991/health
    
    Write-ColorOutput Yellow "`nПроверка ready endpoint:"
    kubectl run curl-test --image=curlimages/curl:latest --rm -i --restart=Never -n $NAMESPACE -- curl -s http://net-as64-220053-v41:7991/ready
    
    Write-ColorOutput Yellow "`nПроверка info endpoint:"
    kubectl run curl-test --image=curlimages/curl:latest --rm -i --restart=Never -n $NAMESPACE -- curl -s http://net-as64-220053-v41:7991/info
...
function Show-Logs {
    ...
    kubectl logs -n $NAMESPACE -l app.kubernetes.io/name=web41 --tail=100
}
...
function Clean-All {
    ...
    helm uninstall web41 -n $NAMESPACE 2>$null
...
function Full-Setup {
    ...
    Write-ColorOutput Yellow "Студент: Смердина Анастасия Валентиновна"
    Write-ColorOutput Yellow "Группа: АС-64, StudentID: 220053, Вариант: 41"
    ...
    Write-ColorOutput Cyan "kubectl port-forward -n $NAMESPACE svc/net-as64-220053-v41 7991:7991"
    Write-ColorOutput Yellow "Затем откройте в браузере: http://localhost:7991"
}
