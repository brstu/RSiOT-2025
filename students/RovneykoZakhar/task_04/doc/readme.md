...
Экспортируемые метрики (с префиксом `app40_`):
- Counter: `app40_http_requests_total{code,method,path}`
- Histogram: `app40_http_request_duration_seconds_bucket{path,method}` (+ `_sum`, `_count`)
...
```bash
helm install app40 ./chart -n app40 --create-namespace
kubectl get pods -n app40
```
