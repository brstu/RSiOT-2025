...
- **Метрики с префиксом app9_:**
  - `app9_requests_total` - счётчик запросов
  - `app9_request_duration_seconds` - гистограмма задержек
...
```dockerfile
LABEL org.bstu.student.fullname="Игнаткевич Кирилл Сергеевич"
LABEL org.bstu.student.id="220042"
LABEL org.bstu.group="АС-64"
LABEL org.bstu.variant="9"
LABEL org.bstu.course="RSiOT"
LABEL org.bstu.owner="pyrokekw"
LABEL org.bstu.student.slug="as64-220042-v9"
```
...
```bash
cd src
docker build -t mon-as64-220042-v9:latest .
```

### 3. Helm чарт

Создан Helm чарт `chart-as64-220042-v9` с параметризацией основных компонентов:
...
```bash
# Создать namespace
kubectl create namespace app-as64-220042-v9

# Установить чарт
helm install as64-220042-v9-app ./src/helm -n app-as64-220042-v9
```
...
kubectl port-forward -n app-as64-220042-v9 svc/mon-as64-220042-v9-service 8080:8080
...
```
# HELP app9_requests_total Total requests
# TYPE app9_requests_total counter
app9_requests_total{endpoint="/",method="GET"} 1.0
# HELP app9_request_duration_seconds Request latency
# TYPE app9_request_duration_seconds histogram
...
```
...
В ходе выполнения лабораторной работы были изучены основные принципы наблюдаемости в Kubernetes. Создано приложение с экспонированием метрик Prometheus через endpoint `/metrics` с префиксом согласно варианту (`app9_`).
