...
- **Метрики с префиксом app37_:**
  - `app37_requests_total` - счётчик запросов
  - `app37_request_duration_seconds` - гистограмма задержек
...
LABEL org.bstu.owner="goryachiy-ugolek"
LABEL org.bstu.student.slug="as64-220050-v37"
```
...
```bash
cd src
docker build -t mon-as64-220050-v37:latest .
```

### 3. Helm чарт

Создан Helm чарт `chart-as64-220050-v37` с параметризацией основных компонентов:
...
```bash
# Создать namespace
kubectl create namespace app-as64-220050-v37

# Установить чарт
helm install as64-220050-v37-app ./src/helm -n app-as64-220050-v37
```
...
kubectl port-forward -n app-as64-220050-v37 svc/mon-as64-220050-v37-service 8080:8080
...
```
# HELP app37_requests_total Total requests
# TYPE app37_requests_total counter
app37_requests_total{endpoint="/",method="GET"} 1.0
# HELP app37_request_duration_seconds Request latency
# TYPE app37_request_duration_seconds histogram
...
```
...
В ходе выполнения лабораторной работы были изучены основные принципы наблюдаемости в Kubernetes. Создано приложение с экспонированием метрик Prometheus через endpoint `/metrics` с префиксом согласно варианту (`app37_`).
